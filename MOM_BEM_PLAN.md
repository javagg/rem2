...
> 项目：rem2 电磁仿真工具  
> 文档版本：v2.0，2026-04-05  
> **实现状态：✅ Phase A-D 全部完成（MoM RWG+CFIE + BEM Laplace P0 + SBR+ 高频 PO）**  
...
- [x] **A1** 创建 `crates/mom/Cargo.toml`，加入 workspace
- [x] **A2** 扩展 `crates/config/src/schema.rs`：
...
- [x] **A3** 更新 `crates/cli/src/main.rs` 的 `match` 分支，添加 `MoM`/`BEM` 路由（返回 `not yet implemented` 错误）
- [x] **A4** 配置解析测试：解析 `examples/sphere_mom.json`（新建示例文件）
...
- [x] **A5** 实现 `crates/mom/src/surface_mesh.rs`：
...
- [x] **A6** 实现 `crates/mom/src/quadrature.rs`：
...
- [x] **B6** 实现 `crates/mom/src/excitation.rs`：
  - `plane_wave_rhs(surf, k, pol, inc_dir) -> Vec<Complex64>`：平面波激励向量
- [x] **B7** 实现 `crates/mom/src/postprocess.rs`：
...
- [x] **B8** 实现 `crates/mom/src/lib.rs`：
  - `pub fn run(config: &PalaceConfig) -> RemResult<()>` 主入口
- [x] **B9** 准备 PEC 球面网格（`examples/meshes/sphere_100.msh`，约 100 面元）
- [x] **B10** 集成测试：`tests/integration/test_mom_pulse.rs`
...
- [x] **C1** 实现 `crates/mom/src/basis/rwg.rs`：
...
- [x] **C2** 实现 `crates/mom/src/assemble.rs` RWG 版（替换脉冲版）：
...
- [x] **C3** 实现 `crates/mom/src/singular.rs` 完整版：
...
- [x] **C5** 实现 `assemble_cfie(surf, freq, bases, alpha, quad) -> faer::Mat<Complex64>`：
  - `Z_CFIE = alpha * Z_EFIE + (1 - alpha) * eta0 * Z_MFIE`
  - 并行装配（`rayon`，按行分块）
- [x] **C6** 更新 `run()` 使用 RWG + CFIE
- [x] **C7** 集成测试：`tests/integration/test_mom_cfie.rs`
...
- [x] **D1** 创建 `crates/bem/` crate（或在 `crates/mom/` 中新增 `src/laplace.rs`）
- [x] **D2** 实现 Laplace Green 函数（已有，无 k 参数的 `green3d` 特例）
- [x] **D3** 实现 P0（常数）基函数版 BEM 算子：
...
- [x] **D4** 施加 Dirichlet 边界条件（PEC/Terminal/Ground）
- [x] **D5** 求解线性系统（`faer::LU`），输出节点电位和法向通量

#### W15：验证与集成

- [x] **D6** 集成测试：`tests/integration/test_bem_electrostatic.rs`
  - 球体电容：`C = 4πε₀R`，误差 < 0.5%
  - 平行板电容（宽板近似）：误差 < 1%
  - 与 FEM 静电求解器交叉验证（同一问题，两种方法误差 < 1%）
- [x] **D7** 输出电容矩阵（与 FEM 静电 `capacitance.csv` 格式相同）
- [x] **D8** 扩展 Yew demo，支持 `BEM` 问题类型示例（可选，按时间决定）
