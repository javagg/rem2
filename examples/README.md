...
> 说明：这是对本仓库示例配置字段完整性的第一轮检查，用于开始 A2 的逐项对齐流程。

| Example Key | Problem | Model | Domains | Boundaries | Solver | 初检结论 |
|---|---|---|---|---|---|---|
| `spheres` | ✅ | ✅ | ✅ | ✅ | ✅ | 字段完整，可进入 Palace 上游逐字段比对 |
| `rings` | ✅ | ✅ | ✅ | ✅ | ✅ | 字段完整，可进入 Palace 上游逐字段比对 |
| `coaxial` | ✅ | ✅ | ✅ | ✅ | ✅ | 字段完整；当前 `Mesh` 指向 `coaxial_ascii.msh`，与 yew 默认值存在差异，需在对齐时统一 |
| `cpw` | ✅ | ✅ | ✅ | ✅ | ✅ | Driven 字段完整；网格多变体策略已定义，后续按模式映射细化 |

下一步执行建议：
1. 为上表每个示例补充 Palace 来源 commit（A2-2）。
2. 逐示例输出“字段差异摘要”（新增/缺失/语义偏移）。
3. 将 `coaxial` 的 yew 默认网格与配置默认网格统一为同一基线。
