...
# Map of expected Rust struct fields to JSON keys
boundaries_fields = {
    "PEC": "pec",
    "PMC": "pmc", 
    "Impedance": "impedance",
    ...
    "Absorbing": "absorbing",
    "Ground": "ground",
    "ZeroCharge": "zero_charge",
    "SurfaceCurrent": "surface_current",
    "Terminal": "terminal",
    # Missing in schema but in configs:
...
driven_fields = {
    "MinFreq": "min_freq",
    "MaxFreq": "max_freq",
    "FreqStart": "FREQ_START_NOT_FOUND",
    "FreqEnd": "FREQ_END_NOT_FOUND",
    "FreqStep": "freq_step",
}
...
for name, config_str in examples.items():
    print(f"\n{'='*60}")
    ...
    if "Boundaries" in cfg:
        ...
        print("Boundaries fields:")
        for key in boundaries:
            if key not in boundaries_fields:
                print(f"  ✗ '{key}' - NOT IN SCHEMA")
            else:
                print(f"  ✓ '{key}'")
    
    # Check Driven Solver
    if "Solver" in cfg and "Driven" in cfg["Solver"]:
        driven = cfg["Solver"]["Driven"]
        print("Driven fields:")
        for key in driven:
            if key not in driven_fields:
                print(f"  ✗ '{key}' - NOT IN SCHEMA")
            elif "NOT_FOUND" in driven_fields[key]:
                print(f"  ✗ '{key}' - FIELD NOT IN SCHEMA (have {driven_fields[key]})")
            else:
                print(f"  ✓ '{key}'")
