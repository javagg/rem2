...
> 版本：2026-04-10  
> REM 基准版本：v0.16.0（`crates/mom/`，249 个测试，零失败）  
...
| **自适应网格** | 无（AMR 在 FEM 求解器中，MoM 无 AMR） | ✅ 自适应 Sonnet 单元细化（基于 S 参数收敛） |
...
| **分层介质基板** | ❌ 仅自由空间 Green 函数 | ✅ **核心优势**：Sommerfeld 积分精确建模任意层叠基板 |
| **有损导体（表面阻抗）** | ❌ MoM 中未实现 | ✅ 表面电阻（σ 有限），R_s 建模 |
...
| **微带传输线 S11/S21** | ❌ MoM 无端口 | < 0.1 dB，工业标准 |
| **贴片天线 S11** | ❌ | < 0.5 dB，业界基准 |
...
3. **S 参数提取与端口建模**：Sonnet 端口建模（集总、波导、差分、去嵌入）是微波电路分析的必备功能，REM MoM 当前无端口激励和 S 参数输出。
...
6. **自适应网格细化（MoM）**：Sonnet 在 MoM 内部支持自适应单元细化；REM MoM 暂无 AMR（AMR 在 FEM 求解器中）。

7. **有损导体建模**：Sonnet 支持有限电导率导体（表面阻抗边界条件），精确建模铜损耗等效 R_s；REM MoM 仅支持 PEC/PMCHWT。
